...
def test_prefix_parser():
    ...
    # Test case 5: Newlines and carriage returns removal
    print("\n5. Testing newlines and carriage returns removal:")
    parser = PrefixParser(["**질문:**", "질문:", "**Question:**", "Question:"])
    
    chunks = ["\n\r질문:\r\n 새로운 사람들과\n", " 편하게 이야기\r\n 나누는 걸 좋아하시나요?\n\r"]
    for i, chunk in enumerate(chunks):
        result = parser.process_chunk(chunk)
        print(f"  Chunk {i+1}: '{repr(chunk)}' -> '{repr(result)}'")
    
    # Test case 6: Prefix in middle of response
    print("\n6. Testing prefix appearing in middle of response:")
    parser = PrefixParser(["**질문:**", "질문:", "**Question:**", "Question:"])
